...
def coherence(
    ...
    *,
    ax: plt.Axes = None,
    plot_properties: dict[str, dict[str, any]] = dict(
        ...
        omitted=dict(
            hatch="///", linewidth=0.4, edgecolor="xkcd:light grey", facecolor=None
        ),
        threshold=dict(
            hatch="\\\\\\", linewidth=0.4, edgecolor="xkcd:light grey", facecolor=None
        ),
    ...
    if ax is None:
        ax = plt.subplots()[1]
    h_smooth = gauss_filter_DataArray(
        waveform.coherence_waveform_20_ku, "ns_20_ku", 35, 35
    ).plot(c=".7", label="smooth")[0]
    h_list = [h_smooth]
    try:
        ...
        h_thr = ax.add_patch(
            mpl.patches.Rectangle(
                (-100, y0),
                waveform.ns_20_ku[-1] + 200,
                waveform.coherence_threshold - y0,
                **plot_properties["threshold"],
                label="threshold",
            )
        )
        h_list.append(h_thr)
    ...
    try:
        if waveform.swath_start[0] > 0:
            ...
            x0 = -100
            h_omitted = ax.add_patch(
                mpl.patches.Rectangle(
                    (x0, -1),
                    waveform.swath_start[0] - x0,
                    3,
                    **plot_properties["omitted"],
                    label="omitted",
                )
            )
    ...
    try:
        ...
        (h_excl,) = ax.plot(
            excluded.ns_20_ku,
            excluded.coherence_waveform_20_ku[0],
            ls="",
            **plot_properties["excluded"],
            label="excluded",
        )
        ...
        (h_swath,) = ax.plot(
            swath.ns_20_ku,
            swath.coherence_waveform_20_ku[0],
            ls="",
            **plot_properties["swath"],
            label="swath",
        )
        ...
        (h_all,) = ax.plot(
            waveform.ns_20_ku,
            waveform.coherence_waveform_20_ku[0],
            ls="",
            **plot_properties["swath"],
            label="all",
        )
    ...
    try:
        ...
        (h_poca,) = ax.plot(
            poca.ns_20_ku,
            poca.coherence_waveform_20_ku[0],
            ls="",
            **plot_properties["poca"],
            label="POCA",
        )
...
def power(
    ...
    *,
    ...
    h_smooth = gauss_filter_DataArray(waveform.P, "ns_20_ku", 35, 35).plot(
        c=".7", label="smooth"
    )[0]
    h_list = [h_smooth]
    try:
        ...
        if logarithmic:
            power_threshold = 10 * np.log10(
                waveform.noise_power_20_ku[0] * waveform.power_threshold[1]
            )
            y0 = -250
        else:
            power_threshold = (
    ...
    try:
        ...
        swath = waveform.where(~waveform.exclude_mask)
        (h_swath,) = ax.plot(
            swath.ns_20_ku, swath.P[0], ls="", **plot_properties["swath"], label="swath"
        )
        h_list.insert(0, h_swath)
    except KeyError:
        (h_all,) = ax.plot(
    ...
    try:
        poca = waveform.sel(ns_20_ku=waveform.poca_idx)
        (h_poca,) = ax.plot(
            poca.ns_20_ku, poca.P[0], ls="", **plot_properties["poca"], label="POCA"
        )
        h_list.insert(0, h_poca)
    except KeyError:
        pass
...
def dem_transect(
    ...
    *,
    ax: plt.Axes = None,
    line_properties: dict[str, dict[str, any]] = dict(
        swath=dict(color="tab:blue", marker=".", markersize=5, linewidth=1),
        poca=dict(color="tab:green", marker="o", markersize=5, linewidth=1),
    ...
    dem_reader = get_dem_reader(
        (waveform if dem_file_name_or_path is None else dem_file_name_or_path)
    )
    trans_4326_to_dem_crs = get_4326_to_dem_Transformer(
        dem_reader.rio.crs if isinstance(dem_reader, xr.DataArray) else dem_reader.crs
    )
    sampling_dist = np.arange(-30000, 30000 + 1, 100)
    num_samples = len(sampling_dist)
    lats, lons = WGS84_ellpsoid.fwd(
        lons=[waveform.lon_20_ku.item()] * num_samples,
    ...
    if not selected_phase_only:
        for ph_idx in waveform.phase_wrap_factor.values:
            ...
            ax.plot(
                ...
                temp.xph_elevs,
                ".",
                c=f"{(0.2 + 0.2 * np.abs(ph_idx)):.1f}",
            )
    best_phase = waveform.sel(
        phase_wrap_factor=waveform.ph_idx
    )  # [["xph_dists", "xph_elevs", "exclude_mask"]]
    try:
        ...
        (h_swath,) = ax.plot(
            ...
            swath.xph_elevs,
            ls="",
            **line_properties["swath"],
            label="swath",
        )
        h_list = [h_swath, h_excl]
    except KeyError:
        (h_all,) = ax.plot(
            best_phase.xph_dists,
            best_phase.xph_elevs,
            ls="",
            **line_properties["swath"],
            label="all",
        )
